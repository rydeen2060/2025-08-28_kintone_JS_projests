(function() {
  ...
  // ========================================
  // UI操作時のレコード保存前処理（submit）
  // ========================================
  kintone.events.on([
    'app.record.create.submit',  // 新規作成時（UI操作）
    'app.record.edit.submit'     // 編集時（UI操作）
  ], function(event) {
    console.log('=== 前回の逸脱判定コピー処理開始（submit） ===');
  ...
  // ========================================
  // API経由（Lambda等）のレコード作成後処理
  // ========================================
  kintone.events.on('app.record.create.submit.success', function(event) {
    console.log('=== 前回の逸脱判定コピー処理開始（API作成後） ===');
    const record = event.record;
    const recordId = event.record.$id.value;

    // 前回の逸脱判定が既に設定されている場合はスキップ
    if (record[FIELD.PREV_DEVIATION] && record[FIELD.PREV_DEVIATION].value) {
      console.log('前回の逸脱判定は既に設定済みのためスキップ');
      return event;
    }
    ...
    // 前回のレコードから逸脱判定を取得
    getPreviousDeviation(currentDatetime, recordId)
      .then(function(prevDeviation) {
        console.log('前回の逸脱判定:', prevDeviation);
        ...
        if (prevDeviation) {
          ...
          return kintone.api(kintone.api.url('/k/v1/record', true), 'PUT', params)
            .then(function(resp) {
              console.log('=== 前回の逸脱判定を更新完了（API） ===', resp);
            })
            .catch(function(error) {
              console.error('更新エラー:', error);
            });
        } else {
          console.log('前回のレコードが見つからないため更新スキップ');
        }
      })
      .catch(function(error) {
